...
g.GremlinJSPipeline.prototype.applyFacets = function (facets) {
    var pipeline = this;
    if (facets) {
        var atstart = true;
        for (var ii = 0; ii < facets.length; ii++) {
            for (var type in facets[ii]) {
                if (!atstart) {
                    pipeline = pipeline.back('me');
                }
                if (type === 'in') {
                    pipeline = pipeline.in(facets[ii][type]);
                } else if (type === 'out') {
                    pipeline = pipeline.out(facets[ii][type]);
                } else {
                    pipeline = pipeline.out(type).filter("{it.key=='" + facets[ii][type] + "'}");
                }
                atstart = false;
            }
        }
...
function connect() {
    ...
    if (engine === 'titan') {
        ...
        try {
            db.makeTypeSync().nameSync("recordtype").unidirectedSync().makeEdgeLabelSync();
            ...
            db.makeTypeSync().nameSync("data").dataTypeSync(Type.String.class).indexedSync("search", Type.Vertex.class)
                .uniqueSync(Direction.OUT).makePropertyKeySync();
            db.makeTypeSync().nameSync("email").dataTypeSync(Type.String.class).indexedSync(Type.Vertex.class)
                .uniqueSync(Direction.IN).makePropertyKeySync();
        } catch (e) {
