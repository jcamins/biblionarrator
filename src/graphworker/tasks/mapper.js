...
module.exports = function (input, callback) {
    ...
    if (source) {
        async.series({
            records: function (cb) {
                g.v(source, function (err, pipe) {
                    pipe.copySplit(g._().out().in(), g._().in().out()).fairMerge().groupCount().cap().orderMap(g.Tokens.decr).range(0, input.size).aggregate(list).toJSON(cb);
                });
            },
            ...
            paths: function (cb) {
                g.v(source, function (err, pipe) {
                    pipe.copySplit(g._().inE().outV().outE().inV(), g._().outE().inV().inE().outV()).fairMerge().retain(list).path('{it->it.id}', '{it->it.inV.next().id + "^" + it.label}', '{it->it.id}', '{it->it.inV.next().id + "^" + it.label}', '{it->it.id}').toJSON(cb);
                });
            }
        }, function (err, res) {
            var records = res.records;
            g.v(source, function (err, pipe) {
                pipe.copySplit(g._().out().in(), g._().in().out()).fairMerge().groupCount().cap().orderMap(g.Tokens.decr).range(0, input.size).aggregate(list).iterate(function () {
                    var edges = [ ];
                    var recmap = { };
                    ...
                    if (Object.keys(needed).length > 0) {
                        ...
                        g.v(Object.keys(needed), function (err, pipe) {
                            pipe.as('records').out('recordtype').property('key').store(recordtypes).optional('records').toJSON(function (err, newrecords) {
