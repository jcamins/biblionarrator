...
function searchTree(tree, pipeline) {
    ...
    case 'FACET':
        ...
        switch (config.schema.indexes[tree[1]].type) {
        case 'property':
            if (typeof optimizer.vertexquery !== 'undefined') {
        ...
        default:
            if (typeof optimizer.vertexquery !== 'undefined') {
                optimizer.gremlinops.push([ config.schema.indexes[tree[1]].type, tree[1], value ]);
            } else {
                return operations[config.schema.indexes[tree[1]].type](pipeline, tree[1], value);
            }
        }
    case 'PHRASE':
        return tree[1];
