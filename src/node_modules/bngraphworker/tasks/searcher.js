...
module.exports = function (input, callback) {
    if (typeof input.query.plan !== 'undefined' && typeof input.query.plan.esquery !== 'undefined') {
        ...
        if (input.query.plan.esonly) {
            ...
            input.query.plan.esquery.size = Math.max(1000, input.query.perpage * 4);
        }
    }
    search(input.query, function (results) {
        if (results.records) {
            ...
            var records = results.pipe.range(input.query.offset, input.query.offset + input.query.size).toJSON();
            var count = input.query.offset + records.length;
