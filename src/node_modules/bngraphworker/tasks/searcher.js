        ...
        environment.esclient.search('bn_moa', 'vertex', plan.esquery, function (err, data) {
            ...
            if (typeof err === 'undefined') {
                ...
                data.hits.hits.forEach(function (hit, index) {
                    ...
                    list.push(rec._id);
                    if (plan.vertexquery.length === 0 && plan.pipeline.length === 0 && index >= input.offset && index < input.offset + input.perpage) {
                        for (var key in hit._source) {
                            rec[environment.esclient.index[key]] = hit._source[key];
                        }
                        records.push(rec);
                    }
                });
...
function handleVQandPipeline(input, plan, list, callback) {
    ...
    if (list === null) {
        if (plan.vertexquery.length > 0) {
            ...
            for (ii = 0; ii < plan.vertexquery.length; ii++) {
                pipe = pipe.hasSync.apply(pipe, plan.vertexquery[ii]);
            }
        ...
        pipe = g.start(list);
        for (ii = 0; ii < plan.vertexquery.length; ii++) {
            pipe = pipe.has.apply(pipe, plan.vertexquery[ii]);
        }
    }
    for (ii = 0; ii < plan.pipeline.length; ii++) {
        pipe = pipe[Object.keys(plan.pipeline)[0]].apply(pipe, plan.pipeline[ii][Object.keys(plan.pipeline)[0]]);
    }
    list = new g.ArrayList();
