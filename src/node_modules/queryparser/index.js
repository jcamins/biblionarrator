...
var queryparser = {
    ...
    parsers: { },
    parse: function (syntax, query) {
        return queryparser.parsers[syntax].parse(query);
    },
...
require("fs").readdirSync(__dirname + '/syntax').forEach(function(file) {
    if (file.indexOf('.js') === file.length - 3 && file !== 'index.js') {
        ...
        queryparser.parsers[parser.name] = parser;
    }
});

function initialize(config) {
    queryparser.config = config;
    console.log(queryparser.config);
    for (var name in queryparser.parsers) {
        queryparser.parsers[name].initialize(config);
    }
    return queryparser;
}
...
var flatteners = {
    ...
    'FACET': function (tree) {
        return {
            facet: [ {
                type: tree[1],
                label: queryparser.config.linktypes[tree[1]].facetlabel,
                value: tree[2].flat,
